...
@implementation NSObject (HGCollection)
    ...
    for ( i = 0; i < max; i++ )
    {
        ...
        else if ( [pathComponent hasPrefix: @"["] )
        {
            ...
            for ( i = 0; i < max; i++ )
            {
                ...
                if ( [arg isEqualToString: @"*"] ) arg = pathArgument;
...
{
    Method newValueForKeyPathMethod = class_getInstanceMethod( [HGCollection class], @selector( valueForKeyPath: ) );
